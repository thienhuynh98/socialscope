...
public class Twitterapicode {
   ...
   * */
  private static ArrayList<Tweet>  search(String searchString, String bearerToken) throws IOException, URISyntaxException {
	    ...
	    if (null != entity) {
          ...
          JSONArray ja = (JSONArray) objSearch.get("data");
          System.out.println("Printing the Json array of statuses");
          System.out.println(ja);
          if (ja != null) {
              for (int i = 0; i < ja.size(); i++)
              {
                  Tweet tweet = new Tweet((((JSONObject)((JSONObject) ja.get(i)).get("user")).get("id").toString()), 
                                          ((JSONObject) ja.get(i)).get("text").toString(), 
                                          (((JSONObject)((JSONObject) ja.get(i)).get("user")).get("referenced_tweets").toString()));
                  tweets.add(tweet);
              }
